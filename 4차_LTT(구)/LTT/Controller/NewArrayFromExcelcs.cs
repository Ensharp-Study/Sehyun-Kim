...
namespace LTT.Controller
{
    internal class NewArrayFromExcelcs
    {
        ...
        public List<string[]> FindDataInExcelRegisterMode(string majorinput, string divideinput, string NameOfClass, string professor, string grade, string classnumber) //MakeNewArrayFromExcel을 통해서 만든 배열을 검사 
        {
            ...
            while (check == true)
            {
                ...
                MenuOfRegisterLecture menu = new MenuOfRegisterLecture();
                Console.SetCursorPosition(0, 185);
                Console.WriteLine("담을 강의 번호를 입력해 주세요. 0을 눌러 종료");
                string inputExpression = Console.ReadLine();
                ...
                foreach (string[] row in searchResult)
                {
                    ...
                    {
                        check = false;
                        menu.ViewMenuOfRegisterLecture(studentData);
                        break;
                    }
